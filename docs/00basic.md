...
启动客户端即可使用命令行与服务器交互。
![alt](./00basic.assets/client_interactive.png)
...
![alt](./00basic.assets/vscode_c++.png)

#### 配置

在configure部分选择支持c++20特性的编译器，如gcc10，clang11等以上版本。

![alt](./00basic.assets/vscode.png){style="zoom:50%"}

这里我们以clang15为例，配置完成后会在Project Outline中出现需要编译的目标文件。

![alt](./00basic.assets/vscode2.png){style="zoom:50%"}

找到wsdb，右键弹出菜单中即可编译或调试。

![alt](./00basic.assets/vscode3.png){style="zoom:50%"}
...
![alt](./00basic.assets/截屏2024-08-02%2010.27.08.png)

在弹出窗口中配置Toolchain。

![alt](./00basic.assets/image-20240802095942890.png)

如果没有可用的Toolchain可点击OK后，在Settings->Build, Execution, Deployment->Toolchain中进一步配置cmake，编译器路径等信息。

![alt](./00basic.assets/image-20240802100150772.png)

配置完成后，在右上角选择需要编译的目标文件，其中wsdb为服务器目标，client为客户端目标

![alt](./00basic.assets/image-20240802100416373.png){style="zoom:50%"}

或直接在菜单的Build一栏中点击Build All即可编译所有目标

![alt](./00basic.assets/image-20240802100507143.png){style="zoom:50%"}

![alt](./00basic.assets/image-20240802100605925.png){style="zoom:50%"}
